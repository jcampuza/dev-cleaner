...
function TerminalButton(
  props: preact.JSX.ButtonHTMLAttributes<HTMLButtonElement> & {
    tone?: "run" | "danger";
  },
) {
  ...
  return (
    <button
      class={`${className ?? ""} ${tone ? `tone-${tone}` : ""}`.trim()}
      type="button"
      {...rest}
    />
  );
...
function App() {
  ...
  const categoryOptions = useMemo(
    () => runtime.scan?.categories ?? [],
    [runtime.scan],
  );
  const selectedBytes = useMemo(
    () =>
      allItems.reduce(
        (sum, item) => (selected.has(item.id) ? sum + item.size : sum),
        0,
      ),
    [allItems, selected],
  );
  ...
  const visibleItems = useMemo(() => {
    ...
    return allItems
      ...
      .filter(
        (item) => categories.size === 0 || categories.has(item.categoryId),
      )
      .filter(
        (item) =>
          !normalized ||
          `${item.label} ${item.path} ${item.reason} ${item.tags.join(" ")}`
            .toLowerCase()
            .includes(normalized),
      )
  ...
  const hydrate = (data: StateResponse) => {
    const scan =
      data.activeOperation === "scan" && data.partialScan
        ? partialToScan(data.partialScan)
        : data.scan;
    const home =
      data.partialScan?.home ??
      data.scan?.summary.home ??
      runtimeRef.current.home;
    const roots =
      data.partialScan?.projectRoots ??
      data.scan?.summary.projectRoots ??
      data.projectRoots;
    setRuntime({
      scan,
      ...
      phase:
        data.activeOperation === "scan"
          ? "scanning"
          : data.activeOperation === "delete"
            ? "deleting"
            : scan
              ? "scan complete"
              : "idle",
      ...
      visited:
        data.partialScan?.index?.entriesVisited ?? runtimeRef.current.visited,
      startedAt:
        data.activeOperation === "scan"
          ? Date.parse(data.partialScan?.startedAt ?? new Date().toISOString())
          : null,
      home,
    });
  ...
  const reconcile = async () =>
    hydrate(await requestJson<StateResponse>("/api/state"));
  ...
  const flushStream = () => {
    ...
    if (items.length > 0)
      setRuntime((value) => ({
        ...value,
        scan: addStreamItems(value.scan, items, value.home),
      }));
  };
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      if (runtimeRef.current.operation) setClock(Date.now());
    }, 250);
    ...
    events.addEventListener("message", (message) => {
      try {
        const event = JSON.parse(message.data) as
          ProgressEvent | { type: "connected" };
        if (event.type === "connected") {
          setRuntime((current) => ({ ...current, connected: true }));
          return;
        }
        const starts =
          event.type === "scan-start" || event.type === "delete-start";
        const current = runtimeRef.current;
        if (
          !starts &&
          event.sessionId &&
          current.operationId &&
          event.sessionId !== current.operationId
        )
          return;
        if (event.type === "scan-start") {
          ...
          setRuntime((value) => ({
            ...
            scan: null,
            operation: "scan",
            operationId: event.sessionId ?? null,
            phase: "scanning",
            visited: 0,
            startedAt: Date.parse(payload.startedAt),
            home: payload.home,
          }));
          ...
          setRuntime((value) => ({
            ...value,
            visited: progress.entriesVisited,
            phase: `indexing ${progress.rootsComplete}/${progress.rootsTotal} roots`,
          }));
        } else if (event.type === "scan-category") {
          setRuntime((value) => ({
            ...value,
            phase: `scanning ${event.message.toLowerCase()}`,
            operationId: event.sessionId ?? value.operationId,
          }));
          ...
          setRuntime((value) => ({
            ...
            operation: "delete",
            operationId: event.sessionId ?? value.operationId,
            phase: event.message.toLowerCase(),
          }));
        } else if (
          ["scan-complete", "delete-complete", "error"].includes(event.type)
        ) {
          setRuntime((value) => ({
            ...value,
            phase:
  ...
  const rootsPayload = (): string[] | null => {
    ...
    const roots = rootsText
      ...
      .filter(Boolean);
    if (roots.length === 0) {
      showToast("enter at least one project folder");
      return null;
    }
    return roots;
  };
  ...
  const runScan = async () => {
    ...
    try {
      const scan = await requestJson<ScanResult>("/api/scan", {
        method: "POST",
        body: JSON.stringify({ projectRoots }),
      });
      setRuntime((current) => ({ ...current, scan, phase: "scan complete" }));
      showToast(
        `${scan.summary.itemCount} items / ${formatBytes(scan.summary.totalSize)}`,
      );
    } catch (error) {
  ...
  const deleteSelected = async () => {
    if (
      selected.size === 0 ||
      !confirm(
        `Delete ${selected.size} selected item(s)? This removes files from disk.`,
      )
    )
      return;
    setRuntime((current) => ({
      ...current,
      operation: "delete",
      phase: "deleting selected items",
    }));
    try {
      const result = await requestJson<DeleteResult & { scan?: ScanResult }>(
        "/api/delete",
      ...
      if (result.scan)
        setRuntime((current) => ({
          ...current,
          scan: result.scan ?? current.scan,
        }));
      setSelected(new Set());
      showToast(
        `freed ${formatBytes(result.freedBytes)}${result.failed.length ? ` / ${result.failed.length} failed` : ""}`,
      );
    } catch (error) {
      showToast(String(error));
    } finally {
  ...
  const elapsed = runtime.startedAt
    ? clock - runtime.startedAt
    : (runtime.scan?.summary.durationMs ?? 0);
    ...
    <main class="terminal">
      <header class="command-bar">
        <div class="identity">
          ...
          <span>dev-cleaner</span>
          <span
            class={`connection ${runtime.connected ? "online" : "offline"}`}
          >
            {runtime.connected ? "connected" : "reconnecting"}
          </span>
        </div>
        <div class="commands">
          <TerminalButton tone="run" disabled={busy} onClick={runScan}>
            [ run scan ]
          </TerminalButton>
          <TerminalButton
      ...
      <section class="telemetry" aria-label="Scan progress">
        <span class="phase">
          <span class="cursor">{busy ? "█" : ">"}</span> {runtime.phase}
        </span>
        <Stat label="found" value={formatBytes(totalSize)} />
        <Stat label="items" value={String(allItems.length)} />
        <Stat
          label="visited"
          value={runtime.visited ? runtime.visited.toLocaleString() : "-"}
        />
      ...
      <section class="control-line" aria-label="Scan and result controls">
        <div class="control-group scope-control">
          <span class="prompt">scope:</span>
          {(["caches", "projects", "home"] as ScopeMode[]).map((mode) => (
            <label class={scope === mode ? "active" : ""}>
              <input
                type="radio"
                name="scope"
                value={mode}
        ...
        {scope === "projects" && (
          <textarea
            ...
            disabled={busy}
            rows={1}
            placeholder="/path/to/work, /path/to/code"
      ...
      <section class="filter-line">
        ...
        {riskOrder.map((risk) => (
          <TerminalButton
            class={`chip ${risks.has(risk) ? "active" : ""} risk-${risk}`}
            aria-pressed={risks.has(risk)}
            onClick={() => setRisks((current) => toggleSet(current, risk))}
          >
            {risk}
          </TerminalButton>
        ))}
        ...
        {categoryOptions.map((category) => (
          <TerminalButton
            class={`chip ${categories.has(category.id) ? "active" : ""}`}
            aria-pressed={categories.has(category.id)}
            onClick={() =>
              setCategories((current) => toggleSet(current, category.id))
            }
          >
            {category.name.toLowerCase()}
          </TerminalButton>
        ))}
        {(risks.size > 0 || categories.size > 0 || queryInput) && (
          <TerminalButton
            class="reset"
            onClick={() => {
              setRisks(new Set());
      ...
      <section class="results-shell" aria-labelledby="results-title">
        <div class="list-head">
          ...
          <span>
            {visibleItems.length}/{allItems.length} entries
          </span>
        ...
        <div
          ...
          ref={listRef}
          tabIndex={0}
          aria-label="Size-sorted cleanable items"
          onScroll={(event) => {
        ...
        >
          {visibleItems.length === 0 ? (
            <div class="empty">
              {busy
                ? "scanning... results will stream here"
                : runtime.scan
                  ? "no entries match"
                  : "select a scope and run scan"}
            </div>
          ) : (
            <div
              class="virtual-space"
              style={{ height: `${visibleItems.length * ROW_HEIGHT}px` }}
            >
      ...
      <footer>
        <span>
          scope_root=
          {runtime.scan?.summary.projectRoots.length
            ? runtime.scan.summary.projectRoots.join(",")
            : "known-caches"}
        </span>
        <span>no items selected by default</span>
      </footer>
      <div
